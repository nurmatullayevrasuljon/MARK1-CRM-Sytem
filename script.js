...
/* ===============================================
   ✅ NASIYA / QISMAN TO'LOVLI SOTUV (YANGI, IXTIYORIY)
   Mavjud handleSale() / addSaleCash / addSaleCard ga
   HECH TEGILMAYDI — bular alohida, faqat foydalanuvchi
   "Mijozga nasiyaga sotish" tugmasini bosganda ishlaydi.
   Backendda tasdiqlangan endpointlar:
     GET  /client/get     (AuthSystem.getClients)
     POST /client/create  (AuthSystem.createClient)
     POST /sale/create    (AuthSystem.createSale — client_id,
                            paid_by_cash, paid_by_card qo'llab-quvvatlaydi;
                            paid < total bo'lsa backend avtomatik qarz
                            (total_remaining) yaratadi — /debt/get orqali
                            ko'rinadi)
=============================================== */
let creditSaleClients = [];

async function loadCreditSaleClients() {
  ...
  try {
    ...
    if (!result || !result.success) {
      console.warn("⚠️ Mijozlar ro'yxati yuklanmadi:", result?.backendMessage);
      return;
    }

    creditSaleClients = Array.isArray(result.data) ? result.data : [];

    select.innerHTML = `<option value="">— Yangi mijoz —</option>` +
      creditSaleClients.map(c =>
        `<option value="${c._id}">${c.client_name}${c.client_phone ? " — " + c.client_phone : ""}</option>`
      ).join("");
  } catch (error) {
...
const toggleCreditSalePanelBtn = document.getElementById("toggleCreditSalePanel");
const creditSalePanel = document.getElementById("creditSalePanel");

if (toggleCreditSalePanelBtn && creditSalePanel) {
  toggleCreditSalePanelBtn.addEventListener("click", () => {
    const isHidden = creditSalePanel.style.display === "none";
    creditSalePanel.style.display = isHidden ? "block" : "none";
    if (isHidden) {
      loadCreditSaleClients();
      updateCreditRemainingPreview();
    }
  });
}

function updateCreditRemainingPreview() {
  const preview = document.getElementById("creditRemainingPreview");
  if (!preview || !saleProduct) return;

  const product = products.find(p => p.id == saleProduct.value);
  const qty = parseFloat(saleQty && saleQty.value) || 0;
  const total = product ? product.price * qty : 0;
  const paid = Number(document.getElementById("creditPaidAmount")?.value) || 0;
  const remaining = Math.max(total - paid, 0);

  preview.value = `${remaining.toLocaleString()} UZS (jami: ${total.toLocaleString()} UZS)`;
}

const creditPaidAmountInput = document.getElementById("creditPaidAmount");
if (creditPaidAmountInput) {
  creditPaidAmountInput.addEventListener("input", updateCreditRemainingPreview);
}
if (saleQty) {
  saleQty.addEventListener("input", updateCreditRemainingPreview);
}
if (saleProduct) {
  saleProduct.addEventListener("change", updateCreditRemainingPreview);
}
...
async function handleCreditSale() {
  ...
  const clientNameInput = document.getElementById("creditClientName");
  const clientPhoneInput = document.getElementById("creditClientPhone");
  const paidAmountInput = document.getElementById("creditPaidAmount");
  const paymentMethodSelect = document.getElementById("creditPaymentMethod");

  const totalAmount = product.price * qty;
  const paidAmount = Math.min(Math.max(Number(paidAmountInput.value) || 0, 0), totalAmount);
  const paymentMethod = paymentMethodSelect.value === "card" ? "card" : "cash";

  let clientId = clientSelect.value || null;

  try {
    // Agar mavjud mijoz tanlanmagan bo'lsa va ism kiritilgan bo'lsa — yangi mijoz yaratamiz
    if (!clientId) {
      const name = clientNameInput.value.trim();
      const phone = clientPhoneInput.value.trim();

      if (!name) {
        showSaleAlert("❌ Mijozni tanlang yoki ismini kiriting!", "error");
        return;
      }

      const createResult = await AuthSystem.createClient({
        client_name: name,
        client_phone: phone || undefined
      });

      if (!createResult || !createResult.success) {
        showSaleAlert(createResult?.backendMessage || "❌ Mijoz yaratilmadi!", "error");
        return;
      }

      clientId = createResult.data?.client?._id;

      if (!clientId) {
        showSaleAlert("❌ Mijoz ID olinmadi!", "error");
        return;
      }
    }
    ...
    const result = await AuthSystem.createSale({
      ...
      client_id: clientId,
      note: currentSaleSessionId.toString(),
      paid_by_cash: paymentMethod === "cash" ? paidAmount : 0,
      paid_by_card: paymentMethod === "card" ? paidAmount : 0
    });

    if (!result || !result.success) {
      showSaleAlert(result?.backendMessage || "❌ Nasiya sotuv amalga oshirilmadi!", "error");
      return;
    }
    ...
    showSaleAlert(
      remaining > 0
        ? `✅ Sotildi! Qarz: ${remaining.toLocaleString()} UZS`
        : `✅ Sotildi! To'liq to'landi.`,
      "success"
    );
    ...
    console.error(error);
    showSaleAlert("❌ Nasiya sotuv amalga oshirilmadi!", "error");
  }
}

const submitCreditSaleBtn = document.getElementById("submitCreditSale");
if (submitCreditSaleBtn) {
  submitCreditSaleBtn.addEventListener("click", handleCreditSale);
}
